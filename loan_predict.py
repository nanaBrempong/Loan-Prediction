...
# Load data
# All preprocessing, feature selection, encoding, and modeling steps are justified with comments

data = pd.read_csv('loan_data.csv')

# 1. Handle missing values by dropping rows with any missing value for simplicity
# (Other strategies like imputation can be used for more advanced cases)
data = data.dropna()
...
# 3. Discretization
# Quantile-based binning for income and loan amount columns for balanced group sizes
# Domain-based binning for Loan_Amount_Term
...
# 6. Feature selection
# Drop original continuous columns and Loan_ID if present
